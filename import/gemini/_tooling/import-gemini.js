...
function isQuestionAdditiveUpdate(existing, candidate) {
  const { passages: existingPassages = [], ...existingRest } = existing;
  const { passages: candidatePassages = [], ...candidateRest } = candidate;
  if (!deepEqual(existingRest, candidateRest)) return false;
...
function buildQuestionEntity(q, existingQuestionFile) {
  if (existingQuestionFile && fs.existsSync(existingQuestionFile)) {
    ...
    const existingPassages = existing.passages || [];
    const merged = existingPassages.slice();
