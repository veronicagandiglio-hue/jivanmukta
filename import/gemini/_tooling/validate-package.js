...
function validatePackageObject(pkg, existing, pkgLabel) {
  ...
  questions.forEach((q, i) => {
    ...
    if (!existing.question[q.id]) {
      ...
      if (q.type && !['specific', 'great', 'comparison'].includes(q.type)) {
        warn(`questions[${i}].type "${q.type}" non è uno dei valori noti (specific, great, comparison).`);
      }
    }
  });
  ...
  const explanations = Array.isArray(pkg.explanations) ? pkg.explanations : [];
  explanations.forEach((exp, i) => {
    if (!exp.id) err(`explanations[${i}].id mancante.`);
    else if (!isKebab(exp.id)) err(`explanations[${i}].id "${exp.id}" non è kebab-case valido.`);
    else checkDuplicateInPackage('explanation', exp.id, `explanations[${i}]`);
    if (!exp.target_type) err(`explanations[${i}].target_type mancante.`);
    if (!exp.target_id) err(`explanations[${i}].target_id mancante.`);
  ...
  explanations.forEach((exp, i) => {
    if (exp.target_type === 'passage' && !passageResolvable(exp.target_id)) {
      err(`explanations[${i}] ("${exp.id}").target_id riferisce passaggio inesistente: "${exp.target_id}"`);
    } else if (exp.target_type === 'question' && !questionResolvable(exp.target_id)) {
      err(`explanations[${i}] ("${exp.id}").target_id riferisce domanda inesistente: "${exp.target_id}"`);
    } else if (exp.target_type === 'concept' && !conceptResolvable(exp.target_id)) {
      err(`explanations[${i}] ("${exp.id}").target_id riferisce concetto inesistente: "${exp.target_id}"`);
    }
  });
